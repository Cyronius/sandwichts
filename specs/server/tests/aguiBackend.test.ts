...
//
// createAguiBackend implements the AG-UI HttpAgent SSE slice over any
// OpenAI-compatible Chat Completions API, with the upstream fetch mocked.
import { describe, it, expect, vi } from 'vitest';
import { createAguiBackend, mapMessages } from '@sandwichts/server';

function openAiSse(deltas: string[]): Response {
    const events = [
        ...deltas.map((d) => JSON.stringify({ choices: [{ delta: { content: d } }] })),
...
describe('SW-AGUI-BACKEND: createAguiBackend', () => {
    it('maps history to a Chat Completions body with inline system, NO tools, and streams AG-UI events', async () => {
        const upstream = vi.fn(async () => openAiSse(['Hello ', 'world']));
        const handler = createAguiBackend({ model: 'gpt-4o-mini', apiKey: 'k', fetchImpl: upstream as any });
    ...
    it('sends no Authorization header without an apiKey and honors a custom baseUrl (local LM)', async () => {
        const upstream = vi.fn(async () => openAiSse(['ok']));
        const handler = createAguiBackend({
            model: 'llama3.2',
            baseUrl: 'http://localhost:11434/v1/', // trailing slash tolerated
            fetchImpl: upstream as any,
        });
        await readAguiEvents(await handler(runInput([{ id: 'u', role: 'user', content: 'go' }])));
    ...
    it('emits a CUSTOM code_mode.script event when enabled and the reply has a js block', async () => {
        ...
        const upstream = vi.fn(async () => openAiSse([reply]));
        const handler = createAguiBackend({ model: 'm', emitScriptEvents: true, fetchImpl: upstream as any });
    ...
    it('emits RUN_ERROR carrying upstream failure text', async () => {
        ...
        const handler = createAguiBackend({ model: 'm', fetchImpl: upstream as any });
        const events = await readAguiEvents(await handler(runInput([{ id: 'u', role: 'user', content: 'go' }])));
    ...
    it('emits RUN_ERROR on an in-stream error payload', async () => {
        const body = `data: ${JSON.stringify({ error: { message: 'model overloaded' } })}\n\n`;
        const upstream = vi.fn(async () => new Response(body, { status: 200 }));
        const handler = createAguiBackend({ model: 'm', fetchImpl: upstream as any });
        const events = await readAguiEvents(await handler(runInput([{ id: 'u', role: 'user', content: 'go' }])));
        expect(events[events.length - 1]).toMatchObject({ type: 'RUN_ERROR', message: 'model overloaded' });
    });

    it('mapMessages keeps system inline, merges consecutive same-role turns, drops empty/tool roles', () => {
